...
the long term objective is to show that $\mathcal{C}$ coincides with the class of generable functions 
...
\begin{proof} It's enough to replace every instruction $J(m,n,t)$ in $P$ such that $t>l(P)+1$ with $J(m,n,l(P)+1)$
\end{proof}
\end{lemma}
...
\begin{remark}
  On the basis of this result we can utilize generalized composition when the $g_i$ are not functions of all the variables or are functions with repetitions.
\end{remark}
